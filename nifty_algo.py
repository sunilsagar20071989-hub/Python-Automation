    ...
    while True:
        try:
            ...
            if is_squareoff_time() and pos_active:
                ...
                logger.info(f"[AUTO SQUARE-OFF 03:10 PM] Exiting {active_symbol} at ₹{ltp:.2f}")

                exit_status = place_order(active_symbol, active_token, "SELL", active_quantity)
            ...
            # ----------------------------------------------------
                ...
                if daily_trades_count >= MAX_DAILY_TRADES:
                    ...
                    logger.info("🛑 2 Consecutive Losses Hit! Halting strategy for today.")
                elif consecutive_win_count >= 3:
                    logger.info("🎉 3 Consecutive Wins Hit! Targets achieved for today.")
                else:
                    ...
                    if signal in ["CE", "PE"]:
                        ...
                        if spot and spot > 0:
                            ...
                            pcr, res_strike, sup_strike = get_option_chain_pcr_and_levels(spot)
                            ...
                            if not (MIN_VIX <= vix <= MAX_VIX):
                                ...
                                sym, tok = get_atm_option_scrip(spot, option_type=signal)
            ...
            # Throttle loop interval based on active state to save API bandwidth
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    # Execute loop if inside trading hours (Mon-Fri, 09:15 to 15:30)
