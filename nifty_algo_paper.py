...
# ==========================================
PAPER_TRADING = True  # True = Virtual / Demo Orders
...
try:
  ...
  print(
      f">>> MODE: {'[PAPER TRADING - ZERO RISK]' if PAPER_TRADING else '[LIVE TRADING]'}"
  )
  print('=' * 70 + '\n')
...
def execute_order(symbol, token, action, qty, price=0.0):
  ...
  if PAPER_TRADING:
    if action == 'BUY':
      ...
      print(
          f'| Symbol: {symbol} | Exit LTP: ₹{price:.2f} | Entry LTP:'
          f' ₹{entry_price:.2f}'
      )
      print(
          f'| Trade PnL: ₹{pnl_total:+.2f} | Overall Paper PnL:'
          f' ₹{total_virtual_pnl:+.2f}'
      )
      print('-' * 60)
      ...
      if close is not None:
        ...
        # 2. VIRTUAL EXIT MONITORING
          ...
          if ltp is not None:
            ...
            print(
                f'[PAPER POS] {active_symbol} | LTP: ₹{ltp:.2f} | Entry:'
