...
def main():
  ...
  send_telegram_alert(
      f"🔍 <b>F&O Stock Master Screener Started!</b> Scanning"
      f" {len(fno_universe)} stocks dynamically..."
  )
  ...
  if qualified_matches:
    ...
    send_telegram_alert(
        f"✅ <b>Scan Complete!</b> Found {len(qualified_matches)} qualified"
        " setup(s)."
    )
  else:
    print(">>> ZERO STOCKS MATCHED STRICT RSI + ROC + EMA FILTERS TODAY.")
    send_telegram_alert(
        "ℹ️ <b>Scan Complete!</b> No stocks matched strict RSI+ROC filters"
        " today."
    )
  print("=" * 95 + "\n")
